...
init python:
    ...
    # Initialize particle system references
    # This needs to be called after adjustable_config and jaily are defined.
    try:
        # Attempt direct call, assuming _particle_system_init_references is globally available
        # from particle_system.rpy's init python block.
        _particle_system_init_references(adjustable_config, jaily)
    except NameError:
        # If direct call fails, try with particle_system namespace.
        if 'particle_system' in globals() and hasattr(globals()['particle_system'], '_particle_system_init_references') and callable(globals()['particle_system']._particle_system_init_references):
            ...
            # Log an error if neither method is available. This helps in debugging.
            renpy.log("CRITICAL: Failed to initialize particle system references. _particle_system_init_references not found directly or in particle_system namespace. Particle effects may not work as intended.")
    ...
    penalty_active = False # This is the global flag for penalty state
    ...
    def update_stats(interacting):
        ...
        # New logic for "trick penalty" activation
        if wrong_input_active and wrong_input_start_time is not None:
            # Use .get for "wrong_grace_period" for safety, defaulting to 5.0 if not found
            grace_period = adjustable_config.get("wrong_grace_period", 5.0)
            if (current_time - wrong_input_start_time) > grace_period:
                penalty_active = True # Activate penalty after grace period
        # If wrong_input_active is False, penalty_active remains as set by rub_timer (usually False)
        # or by the end of a previous penalty cycle.
        ...
        if penalty_active:
            ...
            # Penalty remains active until explicitly reset (e.g., by rub_timer on next valid action/cooldown)
        else:
            # This 'else' block now correctly handles non-penalty state, including during grace period
            if wrong_input_active and wrong_input_start_time is not None: # Still in grace period or just finished
                wrong_elapsed = current_time - wrong_input_start_time
                grace_period = adjustable_config.get("wrong_grace_period", 5.0) # Get it again or pass from above
                transition_duration = adjustable_config.get("wrong_transition_duration", 1.0) / get_hotzone_multiplier()

                if wrong_elapsed < grace_period: # Still in grace period
                    factor = 1.0 
                elif wrong_elapsed < grace_period + transition_duration: # Transitioning out of grace
                    factor = 1.0 - 2.0 * ((wrong_elapsed - grace_period) / transition_duration)
                else: # Grace period and transition are over, but penalty hasn't been activated by the block above (should not happen if logic is correct)
                    factor = -1.0 
                I = factor
            else: # Not a wrong input, or wrong input was resolved
    ...
    class Puzzle: 
        ...
        def process_zone(self, zone, now, expected_zone):
            if self.current_active_zone == zone and zone == expected_zone:
                if self.hold_start_time.get(zone) is None:
                    ...
                    return True # Correct zone, held long enough
            # If it's the correct zone but not held long enough, or wrong zone, or no active zone
            # Reset hold time for this zone if it was being tracked
            elif zone in self.hold_start_time: 
                self.hold_start_time[zone] = None
            return False # Not a successful completion of the step
        ...
        def rub_timer(self):
            ...
            # Cooldown takes precedence and resets penalty flags
            if now < self.cooldown_end:
                self.current_active_zone = None
                wrong_input_active = False
                wrong_input_start_time = None
                penalty_active = False # Ensure penalty is off during cooldown
            ...
            if clicked_cooldown and left_mouse_down():
                ...
                wrong_input_active = False
                wrong_input_start_time = None
                penalty_active = False # Ensure penalty is off during manual cooldown interaction
                return
            ...
            if self.current_active_zone and self.process_zone(self.current_active_zone, now, expected_zone):
                ...
                if self.current_active_zone in self.hold_start_time: # Should be None already from process_zone
                     self.hold_start_time[self.current_active_zone] = None
                
                # Reset wrong input flags and ensure penalty is off
                wrong_input_active = False
                wrong_input_start_time = None
                penalty_active = False 
            else: 
                # This 'else' covers:
                # 1. No active zone (current_active_zone is None)
                # 2. Active zone, but process_zone returned False (e.g., correct zone but not held long enough, or wrong zone)
                if self.current_active_zone and left_mouse_down():
                    # There is an active zone and mouse is pressed
                    if self.current_active_zone != expected_zone:
                        ...
                        penalty_active = False # CRUCIAL: Penalty is NOT active yet, allow grace period
                    else:
                        # It's the CORRECT zone, but not held long enough (process_zone was false)
                        # wrong_input_active and wrong_input_start_time should reflect this is not a "wrong" input state
                        wrong_input_active = False 
                        wrong_input_start_time = None
                        penalty_active = False
                else: 
                    # Mouse is NOT down, or no active zone. Clear wrong input state.
                    wrong_input_active = False
                    wrong_input_start_time = None
                    penalty_active = False # Ensure penalty is off if no interaction

            # Particle update logic (remains unchanged)
            ...
            if 'compute_pps' in globals() and callable(compute_pps) and \
               'particle_config_excitement' in globals() and 'adjustable_config_particles' in globals():
                ...
                if particle_transition_dur == 0: particle_transition_dur = 5.0
        ...
        def reset(self):
            ...
            self.ne1_unlocked = False 
            self.current_active_zone = None
            global wrong_input_active, wrong_input_start_time, penalty_active, jaily, particles, excitement_particles, trust_particles # Added penalty_active here
            wrong_input_active = False
            wrong_input_start_time = None
            penalty_active = False # Ensure penalty is reset
